...
public partial class GetAllCategoriesPage : ComponentBase
{
    ...
    {
        var resut = await Dialog.ShowMessageBox(
            "ATENÇÃO!",
            $"Ao prosseguir, a categoria {title} será excluída. Deseja continuar?",
            yesText: "Excluir",
            cancelText: "Cancelar");

        if (resut is true) await OnDeleteAsync(id, title);
    ...
    private async Task OnDeleteAsync(long id, string title)
    {
        try
        {
            ...
            Snackbar.Add($"A categoria {title} foi removida com sucesso.", Severity.Info);
        }
        catch (Exception ex)
        {
            Snackbar.Add(ex.Message, Severity.Error);
